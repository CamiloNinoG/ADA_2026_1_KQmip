...
class KQNodes(QNodes):
    ...
    def _evaluate_k_partition(self, blocks: List[List[Tuple[int, int]]]) -> Tuple[float, np.ndarray]:
    ...
    def aplicar_estrategia_k(
        ...
        # ── 4. Filtrar: solo candidatos donde CADA bloque tiene ≥1 ACTUAL y ≥1 EFFECT
        candidates_validos = []
        for blocks in candidates:
            if self._bloques_son_validos(blocks, k):
                candidates_validos.append(blocks)

        if not candidates_validos:
            # Fallback: generar k-partición balanceada garantizando ambos tipos
    ...
    def _generar_particion_balanceada(
    ...
    def algorithm(self, vertices: list[tuple[int, int]]):
        ...
        for i in range(len(vertices) - 1):
            ...
            # print("\nFUSIONES:")
            # for l, r, f in self.fusion_history:
            #     print(l, "+", r, "=", f)
